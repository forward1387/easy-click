...
Then(/^I expect the '(.*)' element is( not)? visible$/
	, (locator, visible) => shouldBeVisible(locator, s(visible).trim()===''));
...
Then(/^I expect the element '(.*)' is( not)? checked$/
	, (locator, checked) => checkElementChecked(locator, s(checked).trim()===''));

Then(/^I expect the element '(.*)' is( not)? enabled$/
	, (locator, enabled) => checkElementEnabled(locator, s(enabled).trim()===''));
...
Then(/^I expect the element '(.*)' is( not)? on the page$/
	, (locator, exist) => checkElementExist(locator, s(exist).trim()===''));
