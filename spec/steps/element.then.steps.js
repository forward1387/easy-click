...
Then(/^I expect the '(.*)' element is( not)? visible$/
	, (locator, visible) => shouldBeVisible(locator, visible ? true : false));
...
Then(/^I expect the element '(.*)' is( not)? checked$/
	, (locator, checked) => checkElementChecked(locator, checked ? true : false));

Then(/^I expect the element '(.*)' is( not)? enabled$/
	, (locator, enabled) => checkElementEnabled(locator, enabled ? true : false));
...
Then(/^I expect the attribute '(.*)' from element '(.*)' is not {2}'(.*)'$/
...
Then(/^I expect the element '(.*)' is( not)? on the page$/
	, (locator, exist) => checkElementExist(locator, exist? true: false));
